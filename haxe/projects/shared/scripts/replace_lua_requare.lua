...
local file = io.open("./shared/bin/lua/src/shared.lua", "r")
assert(file)
local contents = file:read("*a")
file:close()

contents = string.gsub(contents, "__lua_lib_luautf8_Utf8%s=%s_G%.require%(\"lua%-utf8\"%)", "local utf8name = \"lua-utf8\"\n__lua_lib_luautf8_Utf8 = _G.require(utf8name)", 1)
contents = string.gsub(contents, "local _hx_bit_raw%s=%s_G%.require%('bit32'%)", "local bit32name = \"bit32\"\n  local _hx_bit_raw = _G.require(bit32name)", 1)
contents = string.gsub(contents, "__lua_lib_luasocket_Socket%s=%s_G%.require%(\"socket\"%)", "__lua_lib_luasocket_Socket = socket", 1)
...
file = io.open("./shared/bin/lua/src/shared.lua", "w")
assert(file)
file:write(contents)
file:close()
return true
