...
function M.activate_camera(cam_id,no_message)
	if cameras[cam_id] then
		if cameras[cam_id] ~= curCam then
			if curCam then curCam.active = false end
			curCam = cameras[cam_id]
			if not no_message then msg.post("@render:", HASHES.MSG_UPDATE_WINDOW) end
		end
	else
...
function M.camera_final(cam_id)
	if curCam == cameras[cam_id] then
		curCam = fallback_cam
		msg.post("@render:", HASHES.MSG_UPDATE_WINDOW)
	end
	cameras[cam_id] = nil
end
...
function M.update_window(newX, newY)
	if curCam then
		...
		for i, v in ipairs(listeners) do
			msg.post(v, HASHES.MSG_WINDOW_UPDATE, { window = M.window, viewport = M.viewport, aspect = curCam.aspectRatio, fov = curCam.fov })
		end
	end
end
