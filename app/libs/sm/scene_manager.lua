...
local function unload_scene(self, scene, new_scene, back)
	...
	local keep_working = not back and new_scene and (scene._config.keep_working or new_scene._config.prev_keep_working)
	if not keep_working then
	...
	if scene._state == STATES.PAUSED and (not modal or back) then
		scene:hide()
	end
...
local function show_new_scene(self, old_scene, new_scene, input, options, back)
	assert(self, "self can't be nil")
	...
	if old_scene then unload_scene(self, old_scene, new_scene, back) end
...
unload_modals = function(self)
	...
	while (true) do
		...
		show_new_scene(self, scene, self.stack:peek(), true, nil, true)
		self.co = co
	end
	self.co = nil
...
function M:show(scene_name, input, options)
	...
	self.co = coroutine.create(function()
		if (scene._state == scene.STATIC.STATES.UNLOADED) then
			load(self, scene)
		end
		...
		local current_scene = (scene._config.modal or options.add_scene) and self.stack:peek() or self.stack:pop()
		show_new_scene(self, current_scene, scene, input, options)
		self.stack:push(scene)
	end)
...
function M:back(input, options)
	...
	self.co = coroutine.create(function()
		...
		show_new_scene(self, prev_scene, assert(self.stack:peek()), input, options, true)
	end)
...
function M:unload_scene(name)
	for i = 1, #self.stack.stack do
		local scene = self.stack.stack[i]
		if (scene._name == name) then
			table.remove(self.stack.stack, i)
			break
		end
	end
	...
	COMMON.coroutine_resume(self.co)
end
...
---@return Scene
function M:scene_get_top()
	return self.stack:peek()
end

function M:scene_stack_have(name)
	for i = 1, #self.stack.stack do
		local scene = self.stack.stack[i]
		if (scene._name == name) then return true end
	end
	return false
end
