...
function View:select_view_area(pos, size, parent)
	...
	local new_size = size + vmath.vector3(70, 70, 0)
	if (new_size.x % 2 == 1) then new_size.x = new_size.x + 1 end
	if (new_size.y % 2 == 1) then new_size.y = new_size.y + 1 end
	local scale_x = (self.texture9.x + self.texture9.z + 6) / new_size.x
	local scale_y = (self.texture9.y + self.texture9.w + 6) / new_size.y
