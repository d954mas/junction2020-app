...
function Script:selection_press(data)
	...
	if (type(data) == "table") then
		...
		self:on_input(COMMON.HASHES.INPUT_TOUCH, { x = new_pos.x, y = new_pos.y, pressed = true })
		...
		COMMON.APPLICATION.THREAD:add(function()
			...
			pcall(function()
				self:on_input(COMMON.HASHES.INPUT_TOUCH, { x = new_pos.x, y = new_pos.y, released = true })
			end)
